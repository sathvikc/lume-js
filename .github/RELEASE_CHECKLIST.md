...
- [ ] Release work (version bump, changelog, any accompanying doc/site fixes) happens on a
      branch, not directly on `main`
- [ ] Branch is up to date with `main`: `git fetch && git status`
- [ ] No uncommitted changes: `git status`
- [ ] All CI checks green on the PR (tests, typecheck, lint, complexity, size)
...
> `npm version` creates a commit and tag automatically. If you prefer manual control, edit
> `package.json` directly and create the tag in step 8 (after merge, not before).
...
- [ ] Double-check the test count against `npm run test -- --run` output at the *end* of
      the branch, not whatever it was when you started writing the entry — it drifts if
      more commits land afterward.
...
> The gh-pages **site** (`gh-pages/index.html`, `home.js`, `compare.js`) pulls version/size/
> test-count from `gh-pages/src/data/metrics.json` automatically via `__LUME_VERSION__` /
> `__LUME_SIZE__` / `__LUME_TESTS__` placeholders, regenerated by
> `scripts/build-site-assets.js` on every site build — no manual edits needed there. If you
> ever hardcode a number directly into a `gh-pages/src/**` file instead of using a
> placeholder, it *will* go stale next release (this happened — see `compare.js`'s bundle
> size row and the footer tagline in `gh-pages/index.html`, both fixed to use
> `__LUME_SIZE__`).

- [ ] `docs/README.md` badge line (`> **Version:** ... **Core:** ... **Tests:** ...`) — also
      manual, also easy to forget. Update it in the same PR.
...
```bash
git add package.json CHANGELOG.md README.md docs/README.md
...
> If you used `npm version` in step 2, the tag already exists on your branch — after the
> PR merges, re-tag the actual merge commit on `main` instead of pushing the branch's tag,
> unless the merge was a fast-forward.
