...
> `npm version` creates a commit and tag automatically. If you prefer manual control, edit `package.json` directly and create the tag in step 9 (after merge, not before).
...
`README.md` badges are **not** auto-generated — update them by hand:

- [ ] `[![Version](.../version-X.Y.Z-orange.svg)]` — bump version
- [ ] `[![Tests](.../tests-NNN%20passing-brightgreen.svg)]` — update test count
- [ ] `[![universal core](.../universal%20core-X.XXKB-blue.svg)]` — `lume-js/state` gzipped size
- [ ] `[![core + DOM](.../core%20%2B%20DOM-X.XXKB-blue.svg)]` — `lume-js` gzipped size

Run `npm run size` to get current gzipped sizes for both entries.
...
- [ ] `docs/README.md` badge line (`> **Version:** ... **Core:** ... **Tests:** ...`) — also manual, also easy to forget. Update it in the same PR.

---

## 5. Regenerate agent doc bundles

```bash
npm run llms
```

**Not optional after a version bump:** the `Version:` line inside `llms.txt` comes from `package.json`, so bumping the version in step 2 makes the committed bundles stale — the CI `agent-docs` job fails the PR until they're regenerated. Commit the updated `llms.txt` / `llms-full.txt` with the release commit (step 7).
...
```bash
git add package.json package-lock.json CHANGELOG.md README.md docs/README.md llms.txt llms-full.txt
...
## 10. Publish to npm — via GitHub Release, not manually
...
1. Wait for npm + CDN propagation (step 11) — the benchmark app pulls from the CDN, not a local build
