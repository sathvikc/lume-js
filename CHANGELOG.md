...
- **Navigation and reference repairs:** duplicate/contradictory nav footer in the arrays tutorial removed; dead `docs/prompts/` link removed; `llms.txt` generator manifest fixed (pointed at two nonexistent API-page paths, omitted all eight handler pages and the installation guide — `npm run validate` failed on it); stale `installation.md` package-exports table completed (`lume-js/state`, `persist`, `on`, …); VISION.md's "Current State"/"Current Versions" sections brought to v2.3 reality.
