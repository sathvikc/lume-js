...
- **`lume-js/state` entry — the universal, DOM-free kernel (1.45 KB gz):**
  ...
  See [docs/changes/11](docs/changes/11-feat-state-entry.md).
...
- **`repeat({ template })` (addons):** declarative list rendering from a
  ...
  See [docs/changes/06](docs/changes/06-feat-template-repeat.md), `examples/template-list/`.
...
- **`state()` / `isReactive()` — reactive brand was dead code:** every store
  ...
  fallback for stores from older versions. See [docs/changes/04](docs/changes/04-fix-reactive-brand.md).
