...
function parse(argv) {
  const a = { modes: ['off', 'pull'], regimes: ['dashboard'], cells: [3000],
    ...
    headed: false, wthrottle: false, paint: 'var', contain: 'off', fixed: false, scroll: false, out: null };
...
const all = [];
console.log(`modes=${a.modes} regimes=${a.regimes} cells=${a.cells} budget=${a.budget} rate=${a.rate}x dot=${a.dot} type=${a.type}${a.typeBursty ? '(bursty)' : ''} text=${a.text} paint=${a.paint} contain=${a.contain} fixed=${a.fixed} scroll=${a.scroll}`);
    ...
    for (const mode of a.modes) {
      ...
      try {
        const r = await runCase({ mode, regime, cells, budget: a.budget, churn: a.churn,
          ...
          paint: a.paint, contain: a.contain, fixed: a.fixed, scroll: a.scroll });
        all.push(r);
