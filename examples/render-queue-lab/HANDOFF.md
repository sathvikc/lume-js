...
9. **Occlusion-driven write culling (Layer 1 for the sweep, not just the paint).** `content-visibility` culls *paint* but every pull mode still sweeps and diffs all N cells per frame — that O(N) sweep is exactly why `smart` alone loses to `off` at 8000 cells (Round 7). Use an `IntersectionObserver` to keep a visible-set; the pull loop diffs/paints visible cells at full rate and visits offscreen cells rarely (or only on scroll). Standards-only, composes with `smartcv`, and is the only listed idea that could raise the ceiling *again* the way `cv` did. Candidate `mode=vis`.
10. **O(changed) dirty-list pull via a kernel `$version` counter.** The sweep is O(N) even when almost nothing changed. Cheapest change-signal that *preserves* the no-subscriber fast-path: a per-store monotonic `$version` bumped in the `set` trap (O(1), no notify — `$`-keys already bypass tracking), so the renderer can skip a store without even reading `.value`. Contrast with the obvious alternative (a beforeFlush hook or `$subscribe` per store), which would *defeat* the Round-6 fast-path — measuring that tension is itself the experiment. Tiny kernel `// EXPERIMENT:` + `mode=dirty`. This also unlocks recency ordering (item 4) for free: drain a dirty ring newest-first with a fairness floor.
...
13. **Productize the winning stack** — once 9–12 settle, decide what ships: the Round-6 fast-path graduates to `main` (settled, just artifact-matrix work); the `smartcv` recipe becomes either a tiny `lume-js/addons` pull-renderer or a docs guide ("high-churn rendering") — a maintainer decision, informed by whether item 10's `$version` makes a generic addon viable at near-zero kernel cost.
...
**Highest-leverage next steps:** (a) graduate the Round-6 fast-path to `main` (behaviourally transparent — 488 tests green — so an artifact-matrix exercise, not more research); (b) of the unstarted research candidates (items 4–13 above), the highest-leverage are **item 9 (occlusion-driven write culling)** — the only idea that could raise the ceiling again the way `cv` did, because every pull mode still sweeps O(N) per frame — and **item 10 (kernel `$version` → O(changed) dirty pull)**, which attacks the same sweep from the data side while preserving the fast-path; **item 12 (paint-floor probe)** is the cheap diagnostic that tells you how much headroom is even left. Items 4–8 remain valid but are lower-leverage refinements.
