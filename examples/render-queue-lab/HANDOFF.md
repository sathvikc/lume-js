...
**Highest-leverage next step:** graduate the Round-6 fast-path to `main` (behaviourally transparent — 488 tests green — so an artifact-matrix exercise, not more research). Remaining unstarted research candidates are items 4–8 below (recency-priority slicing, adaptive-by-measured-rate budget, a clean convergence metric, lighter throttle / real hardware, Web Worker offload) — all lower-leverage than what's now settled.
