...
12. **Paint-floor probe (`mode=canvas`).** The analogue of `plain` for the *other* side: paint all cells to one `<canvas>` (or one text node) with scheduling identical to `pull`, to measure the theoretical paint minimum. Round 7 says paint dominates; this quantifies how much of `smartcv`'s remaining 515 ms is irreducible DOM cost vs. still winnable. Diagnostic only, never shippable — but it decides whether further scheduling research has any headroom at all.
