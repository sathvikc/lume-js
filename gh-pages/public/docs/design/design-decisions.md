...
- ❌ JSX/Template literals → Requires build step, breaks progressive enhancemen
...
// ❌ Won't work
const count = state(0);  // Error: Must pass an objec
```
...
**Alternatives considered:**
- ❌ Only notify on changes → Forces manual initialization, easy to forge
...
- **Standards-based:** Uses native `DOMContentLoaded` even
...
- **No lock-in:** If repeat() doesn't fit, just use JavaScrip
...
- **Standards focus:** `contenteditable` isn't a standard form inpu
