...
document.addEventListener('keydown', (e) => {
  // Only trigger if focus is in an input within focus-lis
...
repeat('#caret-list', caretStore, 'items', {
  ...
  render: (item, el) => {
    if (!el.dataset.initialized) {
      ...
      // Update text without disturbing focus/care
      const input = el.querySelector('input');
...
window.stateTest = {
  reorder: function () {
    ...
    setTimeout(() => {
      ...
      // Debug: Check what elements exis
...
window.startAutomation = async function () {
  ...
  try {
    ...
    // Focus a random input in the focus test lis
...
window.resetAllTests = function () {
  ...
  // Stop auto-update for caret tes
  if (caretAutoInterval) {
